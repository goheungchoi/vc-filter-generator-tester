...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="generate-filters", description="Generate VC project filters"
    )
    ...
    existing_compile_target_paths = compile_target_paths.intersection(
        actual_compile_target_paths
    )
    # print("existing_compile_target_paths", existing_compile_target_paths)
    new_compile_target_paths = actual_compile_target_paths.difference(
    ...
    for item_group in proj_item_groups[:]:
        if (
            item_group.get("Label") != None
            and item_group.get("Label") == "ProjectConfigurations"
        ):
            continue
